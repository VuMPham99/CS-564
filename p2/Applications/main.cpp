...
PageId pid[num], pageno1, pageno2, pageno3, pageno4,pageno6 ,i;
RecordId rid[num], rid2, rid3, rid4, rid6;
...
void test8()
{
	//unpin all, flush, allocate, unpin
	for (i = 0; i < num; i++)
	{
...
void test12(){ //test disposePage
	...
	//dipose allocated pages
	for (i = 0; i < num; i++){
		bufMgr->disposePage(file1ptr, pid[i]);
	}
	//try to read a disposed page
	//should not be able to read since it was disposed
	try{
